...
func loadTablesAndFinalize(ctx context.Context, opts ImportOptions, destURI, sqlitePath string, timings *ImportTimings, resume bool) error {
	...
	opts.reportProgress(ImportProgress{Stage: ImportStageForeignKeys})
	if err := applyPostgresForeignKeys(ctx, opts, destURI); err != nil {
		return err
	}
...
func applyPostgresForeignKeys(ctx context.Context, opts ImportOptions, destURI string) error {
	parts, _, err := ConvertSchemaParts(opts.InputPath)
	if err != nil {
		return err
	}
	if strings.TrimSpace(parts.ForeignKeys) == "" {
		return nil
	}

	workDir, err := os.MkdirTemp("", "pscale-d1-fks-*")
	if err != nil {
		return err
	}
	defer os.RemoveAll(workDir)
	...
	var b strings.Builder
	b.WriteString("-- Generated by pscale import d1 (post-load foreign keys)\n")
	b.WriteString(parts.ForeignKeys)

	fkPath := filepath.Join(workDir, fmt.Sprintf("postgres-foreign-keys-%s.sql", opts.MigrationID))
	if err := os.WriteFile(fkPath, []byte(b.String()), 0o600); err != nil {
		return err
	}

	return runPsqlFile(ctx, destURI, fkPath)
}
