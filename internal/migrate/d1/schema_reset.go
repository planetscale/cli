...
func reassignStaleImportRoleObjects(ctx context.Context, psClient *ps.Client, opts ImportOptions, currentUsername string) error {
	...
	for _, role := range roles {
		...
		err := psClient.PostgresRoles.ReassignObjects(ctx, &ps.ReassignPostgresRoleObjectsRequest{
			Organization: opts.Org,
			Database:     opts.Database,
			Branch:       opts.Branch,
			RoleId:       role.ID,
		...
		if err != nil && firstErr == nil {
			firstErr = fmt.Errorf("reassign objects from role %q to %s: %w", role.Username, postgresRoleName, err)
		}
	}
	return firstErr
}
