...
func DoctorNextSteps(result *DoctorResult) []NextStep {
	...
	return []NextStep{
		{Tool: "import_d1_export", Command: "pscale import d1 export --d1-database <name> --remote", Reason: "Export D1 database remotely"},
		{Tool: "import_d1_lint", Command: "pscale import d1 lint --input ./d1-export.sql", Reason: "Lint an existing export file"},
	}
}

// FindWrangler returns the wrangler command to execute.
func FindWrangler() (string, []string, error) {
	if path, err := execabs.LookPath("wrangler"); err == nil {
		return path, nil, nil
	}
	if path, err := execabs.LookPath("npx"); err == nil {
		return path, []string{"wrangler"}, nil
	}
	return "", nil, errMissingTool("wrangler", wranglerMissingRemediation)
}
