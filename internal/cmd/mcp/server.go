		...
		},
		{
			tool: mcp.NewTool("get_schema",
				...
				mcp.WithString("tables",
					mcp.Description("Tables to get schemas for. MySQL: single name, comma-separated list, or '*' for all tables. PostgreSQL: supports '*' (all tables in all schemas), 'schema.*' (all tables in schema), 'schema.table' (specific table), or 'table' (defaults to public.table)"),
					mcp.Required(),
				),
				mcp.WithString("org",
					mcp.Description("The organization name (uses default organization if not specified)"),
