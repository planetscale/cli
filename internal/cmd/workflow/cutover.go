...
func CutoverCmd(ch *cmdutil.Helper) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			end := ch.Printer.PrintProgress(fmt.Sprintf("Starting cutover for workflow %s in database %s...", printer.BoldBlue(number), printer.BoldBlue(db)))
			defer end()
			...
			if ch.Printer.Format() == printer.Human {
				ch.Printer.Printf("Cutover started for workflow %s in database %s.\nCurrent workflow state: %s\n",
