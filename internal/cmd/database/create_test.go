...
func TestDatabase_CreateScalerPro(t *testing.T) {
	c := qt.New(t)
	...
	svc := &mock.DatabaseService{
		CreateFn: func(ctx context.Context, req *ps.CreateDatabaseRequest) (*ps.Database, error) {
			c.Assert(req.Organization, qt.Equals, org)
			c.Assert(req.Name, qt.Equals, db)
			c.Assert(req.Plan, qt.Equals, ps.Plan("scaler_pro"))
			c.Assert(req.ClusterSize, qt.Equals, ps.ClusterSize("PS_20"))
	...
	cmd := CreateCmd(ch)
	cmd.SetArgs([]string{db, "--plan", "scaler_pro", "--cluster-size", "PS_20"})
	err := cmd.Execute()
