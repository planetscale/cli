...
func CreateCmd(ch *cmdutil.Helper) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			into := flags.deployTo
			if len(into) == 0 && len(flags.into) > 0 {
				into = flags.into
			}
	...
	cmd.PersistentFlags().StringVar(&flags.deployTo, "deploy-to", "", "Branch to deploy into. By default, it's the parent branch (if present) or the database's default branch")
